...
| --- | --- | --- | --- |
| `datasource` | Implementado | `DatasourceToolResources` agrupa `metadata.resources` por carpeta. En `MessageParts`, `AggregatedSourceResults` combina Datasource, Perplexity y Firecrawl, muestra hasta tres cards y añade un trigger `+N` con un sheet completo. Las ejecuciones sin recursos no renderizan ninguna card. Los recursos almacenados sin URL pública resuelven una URL firmada al hacer clic mediante `getResourceDownloadUrl`; la URL privada del bucket nunca se renderiza como enlace. | Mantener el conteo después de filtrar privados y duplicados, conservar ocultos los resultados vacíos y resolver siempre las URLs firmadas bajo demanda por su corta duración. |
...
| `structured_datasource` | No planificado (fallback) | Fallback JSON. | Se mantiene con el fallback genérico; no requiere card dedicada mientras el backend solo exponga una respuesta textual. |
